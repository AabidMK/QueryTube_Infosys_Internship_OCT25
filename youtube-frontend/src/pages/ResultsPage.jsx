...
export default function ResultsPage() {
  ...
  // Track open/closed summary for each result
  const [showSummary, setShowSummary] = useState({});

  const toggleSummary = (rank) => {
    setShowSummary((prev) => ({
      ...prev,
      [rank]: !prev[rank],
    }));
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      setResults(res.data);
      setShowSummary({}); // reset toggles after search
    } catch (err) {
      console.error(err);
                  ...
                  >
                    <div style={{ display: "flex", gap: 12 }}>
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        <div className="small">
                          Views: {Number(item.views).toLocaleString()} • Likes:{" "}
                        ...
                        <button
                          ...
                          onClick={() => toggleSummary(item.rank)}
                        >
                          {showSummary[item.rank] ? "Hide Summary" : "Show Summary"}
                        </button>

                        {/* ---- SUMMARY BOX ---- */}
                        {showSummary[item.rank] && (

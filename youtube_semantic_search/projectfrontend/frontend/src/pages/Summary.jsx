  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#0a1128] via-[#1a1f3a] to-[#0a1128] pt-24 pb-16 px-6">
      <div className="max-w-5xl mx-auto">
          ...
          <div className="max-w-2xl mx-auto">
            <div className="flex gap-4">
              <Input
                ...
                className="w-full px-6 py-6 text-lg bg-[#1a1f3a] border-cyan-500/30 text-white"
              />

              <Button
                onClick={() => handleGenerate()}
                disabled={isGenerating}
                className="px-8 py-6 text-lg font-semibold bg-cyan-500 hover:bg-cyan-600 text-white"
              >
        ...
        {/* SUMMARY BOX */}
        {summary && (
          <div className="bg-[#1a1f3a] border border-cyan-500/20 p-10 space-y-10">

            {/* TITLE */}
            <div>
              <p className="text-cyan-400 text-lg font-semibold">Title :</p>
              <h2 className="text-3xl font-bold text-white mt-2">{summary.title}</h2>
            </div>

            {/* CHANNEL */}
            <div>
              <p className="text-cyan-400 text-lg font-semibold">Channel :</p>
              <p className="text-gray-300 text-xl mt-2">{summary.channel}</p>
            </div>

            {/* SUMMARY POINTS */}
            <div>
              <p className="text-cyan-400 text-lg font-semibold mb-4">Summary :</p>

              <div className="space-y-6">
                {summary.summary_points.map((point, index) => (
                  <div key={index} className="flex gap-4 group">
                    <div className="w-8 h-8 bg-cyan-400/10 border border-cyan-400/30 
                                    text-cyan-400 font-semibold flex items-center justify-center rounded">
                      {index + 1}
                    </div>
            ...
            <div className="flex justify-center">
              <Button
                ...
                variant="outline"
                className="px-8 py-4 bg-transparent border-2 border-cyan-400/50 text-cyan-400 hover:bg-cyan-400/10"
              >
                Generate Another
              </Button>
            </div>
